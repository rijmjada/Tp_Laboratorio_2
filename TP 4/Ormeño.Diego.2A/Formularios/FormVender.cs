...
namespace Formularios
{
    public partial class FormVender : Form
    {
        ...
        public FormVender(Deposito<Producto> miDeposito, List<Venta> listVentas)
        {
            InitializeComponent();
        ...
        private void btnVender_Click(object sender, EventArgs e)
        {
            if(listProductoSeleccionado.SelectedItem != null)
            {
                try
                {
                    if (AccesoBD.Ventas_BD.Agregar(new Venta(DateTime.Now, Convert.ToDouble(((Producto)listProductoSeleccionado.SelectedItem).Precio))))
                    {
                        AccesoDatos_BD.Stock_BD.Eliminar(((Producto)listProductoSeleccionado.SelectedItem));
